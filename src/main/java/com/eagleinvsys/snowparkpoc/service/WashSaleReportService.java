...
public class WashSaleReportService
{
    ...
    private static List<FinalSummaryReport> generateFinalSummaryReport(List<SummaryRecord> shortTermSummaryRecords, List<SummaryRecord> longTermSummaryRecords)
    {
        ...
        String reportStartDate = "20220531";
        String reportEndDate = "20220531";
    ...
    private static List<SummaryRecord> getLongTermSummaryRecords(ReversalAdjustment reversalAdjustment, GLSum glSum, BookCostBase bookCostBase, List<SummaryRowData> dfFifthQuery)
    {
        ...
        longTermSummaryRecords.add(SummaryRecord.builder().summaryTitle("Long Term Capital Loss")
                                                .value(BigDecimal.ZERO.subtract(dfFifthQuery.stream()
        ...
        List<SummaryRowData> dfFifthQuery)
    {
        ...
        shortTermSummaryRecords.add(SummaryRecord.builder().summaryTitle(dfFifthQuery.get(0).getTitle3())
                                                 .value(BigDecimal.ZERO.stripTrailingZeros())
                                                 .build());

        shortTermSummaryRecords.add(SummaryRecord.builder().summaryTitle("ST Capital Deferrals")
                                                 .value(BigDecimal.ZERO.stripTrailingZeros()) // todo - not calculated
                                                 .build());
        ...
        shortTermSummaryRecords.add(SummaryRecord.builder().summaryTitle("Short Term Capital Loss")
                                                 .value(BigDecimal.ZERO.subtract(  // todo - using zero for missing v_ws_s_def_b
                                                     dfFifthQuery.stream()
        ...
        shortTermSummaryRecords.add(SummaryRecord.builder().summaryTitle("Converted Wash Sales - Fund Merger")
                                                 .value(BigDecimal.ZERO) // todo - use zero for missing field v_ws_def_merger_b)
                                                 .build());
        ...
        // (NVL(v_prior_def_b,0)-NVL(v_prior_rev_b,0) + NVL(v_ws_def_merger_b,0) +
        //            (NVL(v_ws_s_def_b_m,0)-(v_sum_rev_adjst_s_b+SUM(t.cur_pr_st_reversal_b + t.cur_pr_inkind_st_rev_b) )) +
        //            (NVL(v_ws_l_def_b_m,0)-(v_sum_rev_adjst_l_b+SUM(t.cur_pr_lt_reversal_b + t.cur_pr_inkind_lt_rev_b) )))
        BigDecimal currentPeriodLTReversalBase = dfFifthQuery.stream()
        ...
        shortTermSummaryRecords.add(SummaryRecord.builder().summaryTitle("Post 30 Day Wash Sale Deferrals")
                                                 .value(BigDecimal.ZERO) // todo not available v_post_def_b
                                                 .build());
